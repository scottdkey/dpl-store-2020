...
import { Image, Button, Icon, Checkbox, Modal } from 'semantic-ui-react'
...
export default class AdminCard extends React.Component {
  ...
  render() {
    const { product, deleteProduct, toggleForm } = this.props
    ...
    return (
      <div key={`product-${product.id}`}>
        <div>
          ...
          <div onClick={()=>{deleteProduct(product.id, product.category_id)}} style={{ ...style.button, color: '#990000' }}><Icon name='trash alternate' /></div>
          <div onClick={this.toggleEdit} style={{ ...style.button, color: '#4575c4' }}><Icon name="edit" /></div>
          <div>
            <input 
              type='checkbox'
